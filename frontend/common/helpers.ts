...
export type NoNullableFieldsRecursive<ObjectType, Exclude=never> = {
  [K in keyof ObjectType]-?: ObjectType[K] extends Exclude ? ObjectType[K] : (ObjectType[K] extends object
    ? NoNullableFieldsRecursive<ObjectType[K], Exclude>
    : NonNullable<ObjectType[K]>)
};

export type NonNullableRecursive<AnyType, Exclude=never> = AnyType extends Exclude ? AnyType : (NonNullable<AnyType extends object ? NoNullableFieldsRecursive<AnyType, Exclude> : AnyType>);
